...
void   Z80CPU::setFlag(const flag::StatusFlag statusFlag, bool value)
{
...
void  Z80CPU::add_a_r(uint8_t reg)
{
...
void  Z80CPU::add_hl(uint8_t reg)
{
...
uint8_t  Z80CPU::inc(const uint8_t value)
{
...
uint8_t  Z80CPU::dec(const uint8_t value)
{
	uint8_t result = value - 1;
	setFlag(flag::S, static_cast<int8_t>(result) < 0);
	setFlag(flag::Z, result == 0);
...
void  Z80CPU::ld_de(uint8_t value)
{
...
void  Z80CPU::ld(const uint16_t addressReg, uint8_t src)
{
	ram.write(addressReg, src);
	_registers.pc += 1;
}

void  Z80CPU::ld_acc_address(uint16_t addressReg)
{
...
void  Z80CPU::jr(const bool jump_condition)
{
	_registers.pc += 2;
	uint8_t offset = ram.read(_registers.pc + 1);
	if (jump_condition) {
